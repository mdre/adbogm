...
public abstract class SObject {
    ...
    protected final int validate(Map<String, Integer> acls, ISecurityCredentials sc) {
        LOGGER.log(Level.DEBUG, "validando los permisos de acceso...");
        int partialState = 0;
        Integer gal = 0;
        LOGGER.log(Level.DEBUG, "Lista de acls: {} : {}", new Object[]{acls.size(), acls});
        boolean hasGal = false;
        if (!acls.isEmpty()) {
            for (String securityCredential : sc.showSecurityCredentials()) {
                gal = acls.get(securityCredential);
                LOGGER.log(Level.DEBUG, "SecurityCredential access: {} {}", new Object[]{securityCredential, gal});
                if (gal != null) {
                    hasGal = true;
