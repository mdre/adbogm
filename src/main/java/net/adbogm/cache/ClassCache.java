...
import net.adbogm.annotations.Embedded;
...
public class ClassCache {
    ...
    public ClassDef get(Class<?> c) {
        LOGGER.log(Level.DEBUG, "Procesando clase: {}", c.getName());
...
//        }
        ...
        if (cached == null) {
            LOGGER.log(Level.DEBUG, "Nueva clase detectada. Analizando...");
            cached = this.cacheClass(toProcess);
            this.classCache.put(toProcess, cached);
        }
        LOGGER.log(Level.DEBUG, "Class struc:");
        LOGGER.log(Level.DEBUG, "Class: " + toProcess.getName());
        LOGGER.log(Level.DEBUG, "Fields: " + cached.fields.size()+ " - " + cached.fields);
        LOGGER.log(Level.DEBUG, "enums: " + cached.enumFields.size()+ " - " + cached.enumFields);
        LOGGER.log(Level.DEBUG, "Links: " + cached.links.size()+ " - " + cached.links);
        LOGGER.log(Level.DEBUG, "LinkList: " + cached.linkLists.size()+ " - " + cached.linkLists);
        LOGGER.log(Level.DEBUG, "Indirect Link: " + cached.indirectLinks.size()+ " - " + cached.indirectLinks);
        LOGGER.log(Level.DEBUG, "Indirect LinkList: " + cached.indirectLinkLists.size()+ " - " + cached.indirectLinkLists);
    ...
    private void cacheClass(Class<?> c, ClassDef cached) {
        if (c != Object.class  && !c.getName().startsWith("java")) {
            LOGGER.log(Level.DEBUG, "Clase: {}", c.getName());
            ...
            LOGGER.log(Level.DEBUG, "Analizando superclass...");
            ...
            for (Field f : fields) {
                try {
                    ...
                    if (!(f.isAnnotationPresent(Ignore.class)
                        ...
                        LOGGER.log(Level.DEBUG, "Field: {}  Type: {}{}",
                        ...
                        if (PRIMITIVE_MAP.get(f.getType()) != null) {
                            ...
                            LOGGER.log(Level.DEBUG, "Colección detectada: {}", f.getName());
                            boolean setAsEmbedded = false;
                            boolean isEnumCollection = false;
                            if (List.class.isAssignableFrom(f.getType())) {
                                LOGGER.log(Level.DEBUG, "Se trata de una Lista...");
                                ...
                                if (listClass.isEnum()) {
                                    LOGGER.log(Level.DEBUG, "Es una colección de enums: {}", typeName);
                                    isEnumCollection = true;
                                } else if ((Primitives.PRIMITIVE_MAP.get(listClass) != null)
                                        || (f.isAnnotationPresent(Embedded.class))) {
                                    LOGGER.log(Level.DEBUG, "\n**********************************************************");
                                    if (f.isAnnotationPresent(Embedded.class)) {
                                        LOGGER.log(Level.DEBUG, "Clase anotada como Embedded: {}", typeName);
                                    } else {
                                        LOGGER.log(Level.DEBUG, "Es una colección de primitivas: {}", typeName);
                                        LOGGER.log(Level.DEBUG, "Se procede a embeberla.");
                                    }
                                    LOGGER.log(Level.DEBUG, "\n**********************************************************");
                                    setAsEmbedded = true;
                                }
                                ...
                                // sea una primitiva
                                LOGGER.log(Level.DEBUG, "se trata de un Map...");
                                ParameterizedType listType = (ParameterizedType) f.getGenericType();
                                ...
                                if ((keyClass == String.class)
                                        ...
                                        || (f.isAnnotationPresent(Embedded.class)))) {
                                    LOGGER.log(Level.DEBUG, "Es una colección de embebida: {}", valClass.getSimpleName());
                                    setAsEmbedded = true;
                                }
                            }
                            ...
                            if (setAsEmbedded) {
                                ...
                                LOGGER.log(Level.DEBUG, "Es una colección de objetos que genera Vértices y Ejes.");
                            ...
                            if (f.isAnnotationPresent(Indirect.class)) {
                                ...
                                LOGGER.log(Level.DEBUG, "Link detectado!");
                        ...
                        if (!(f.isAnnotationPresent(Ignore.class))) {
                            if (f.getName().startsWith("___ogm___")||f.getName().startsWith("___tdd___")) {
                                LOGGER.log(Level.DEBUG, "Ignorado: {}", f.getName());
                            } else {
                                LOGGER.log(Level.WARN, "Ignorado: {}", f.getName());
                            }
                        }
                    }
            ...
            LOGGER.log(Level.DEBUG, "Fin clase {} <<<<<<<<<<<<<<<<<", c.getName());
        }
    }
